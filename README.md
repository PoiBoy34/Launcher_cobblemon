...
Launcher Minecraft dédié au modpack **Aventure Cobblemon** (Minecraft 1.21.1, fabric).
...
👉 **https://github.com/PoiBoy34/Launcher_cobblemon/releases/latest**

### Windows
1. Télécharge **`Cobblemon-Launcher-Setup-x.x.x.exe`**
2. Lance-le et suis l'installation
3. Ouvre **Cobblemon-Launcher** depuis le menu Démarrer ou le raccourci bureau

### Linux
1. Télécharge **`Cobblemon-Launcher-x.x.x.AppImage`**
2. Rends-le exécutable (clic droit → Propriétés → Autoriser l'exécution, ou
   `chmod +x Cobblemon-Launcher-*.AppImage`)
...
```powershell
Get-FileHash .\Cobblemon-Launcher-Setup-x.x.x.exe -Algorithm SHA256
```
...
# Cobblemon-Launcher (English)

A Minecraft launcher dedicated to the **Aventure Cobblemon** modpack (Minecraft 1.21.1,
fabric). Install it once, log in, hit **Play** — mods, configs, resource
...
- **Windows**: download **`Cobblemon-Launcher-Setup-x.x.x.exe`** and run it.
- **Linux**: download **`Cobblemon-Launcher-x.x.x.AppImage`**, `chmod +x` it, run it.
...
| `modpacks/Cobblemon/` | mods, datapacks, shaders, resource packs + manifests |
...
```bash
cp mon-nouveau-mod.jar modpacks/Cobblemon/mods/
...
```bash
split -b 90M -d -a 2 "modpacks/Cobblemon/mods/gros.jar" \
                     "modpacks/Cobblemon/mods/gros.jar.part"
rm "modpacks/Cobblemon/mods/gros.jar"
```
