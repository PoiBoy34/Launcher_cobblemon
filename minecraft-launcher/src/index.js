// ===========================================================================
// Cobblemon-Launcher — process principal Electron
...
require('./migrate')(app, ['SUS-Launcher','sus-launcher'], ['instances/pack_cobblemon','java/jre21'], ['msmc-auth.json']);
...
app.whenReady().then(() => {
    log.info('=== Cobblemon-Launcher v' + app.getVersion() + ' démarré (' + process.platform + ' ' + os.release() + ') ===');
    createWindow();
    setTimeout(() => {
...
async function buildDiagnosticReport(packData) {
    const lines = [];
    const pack = packData || {};
    lines.push('===== RAPPORT COBBLEMON-LAUNCHER =====');
    ...
    const hostsToTest = [
        ...
        pack.server_host ? pack.server_host.split(':')[0] : 'cobblemon.mekhorizon.org',
        (pack.voice_test || 'nih-barefoot.tun.ply.gg:1059').split(':')[0]
    ];
...
ipcMain.handle('save-diagnostics', async (event, packData) => {
    try {
        ...
        const { filePath } = await dialog.showSaveDialog(currentWindow, {
            ...
            defaultPath: path.join(app.getPath('desktop'), 'cobblemon-launcher-rapport.txt'),
...
ipcMain.handle('test-voicechat', async (event, packData) => {
    const pack = packData || {};
    const voiceTarget = pack.voice_test || 'nih-barefoot.tun.ply.gg:1059';
    const [voiceHost, voicePortStr] = voiceTarget.split(':');
    const voicePort = parseInt(voicePortStr || '24454');
    const mcTarget = pack.server_host || 'cobblemon.mekhorizon.org:25565';
